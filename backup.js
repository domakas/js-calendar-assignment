(() => {
    ...
    Calendar.EventsMain = {
        ...
        const monthdaysUL = document.querySelector('.grid__header__monthdays');
        const cellHeight = 50;
        const gridItemsArr = [];
        let selected;
        let tempGridItem;
        ...
        monthdaysUL.addEventListener('click', openEventModal);
        ...
        function addEventToCalendar(calendarEvent) {
            ...
            function setEventPosition() {
                ...
                if (isOverlaping(getVisibleDate(), eventRange)) {

                    let startingPoint = getVisibleDate().startDate > eventRange.startDate ? getVisibleDate().startDate : eventRange.startDate;
                    let endingPoint = getVisibleDate().endDate < eventRange.endDate ? getVisibleDate().endDate : eventRange.endDate;

                    let dateObjStart = new Date(startingPoint);
                    let dateObjEnd = new Date(endingPoint);

                    let eventStartDay = dateObjStart.getDay();
                    let eventEndDay = dateObjEnd.getDay();
                    let startDayPosition = (eventStartDay === 0) ? 6 : (eventStartDay - 1);
                    let endDayPosition = (eventEndDay === 0) ? 7 : (eventEndDay);
                    ...
                    let positionLeft = 100 / 7 * startDayPosition;
                    ...
                    if (eventStartDay !== eventEndDay) {
                        ...
                        eventElement.style.backgroundColor = '#438ae7';
                        eventElement.style.width = "calc(100% / 7 *" + ` ${(endDayPosition - eventStartDay)+1})`;
                        monthdaysUL.appendChild(eventElement);
                    } else {
                        eventElement.style.left = `${positionLeft}%`;
        ...
        function displayEventsFromStorage() {
            ...
            eventsInStorage.forEach((calendarEvent) => addEventToCalendar(calendarEvent));
        }
        ...
        function isOverlaping(rangeA, rangeB) {
